...
static EVENT_HANDLER(physical_ready)
{
    ...
    if (nodeinfo.nodetype == NT_HOST) {
      /*
	This if statement is used to segregate the original code
	from our editted code. This is used as a reference as well 
	as it is used for the hosts
      */
      switch (f.kind) {
      case DL_ACK :
	...
	break;
	  ...
	  printf("down from application, seq=%d\n", nextframetosend);
	  transmit_frame(&f.msg, DL_DATA, len, nextframetosend);
	  nextframetosend = 1-nextframetosend;
	}
        else
          printf("ignored\n");
